---
name: revisor-estilo
description: Revisa y corrige el estilo de prosa (em dashes, referencias por libro en vez de por capítulo, verbosidad de LLM, términos en inglés sin su equivalente en español antes) y la notación matemática (mayúscula solo para variables aleatorias, minúscula para todo lo demás; sin punto de multiplicación "·"/`\cdot`, yuxtaponer o usar "×"; `π` reservada para la constante matemática, tasas de inflación como `r_{inf}`; valores esperados/predichos con `\hat{}`, no `^e`) en archivos de notas_unidades/**/*.md o practicas/**/*.md ya escritos, según las reglas de "Estilo de prosa" y "Notación matemática" del skill notas-unidad. Úsalo cuando el usuario pida revisar, limpiar o pulir el estilo de una nota existente, no para redactar contenido nuevo (para eso usa notas-unidad).
---
...
6. **Punto de multiplicación ("·" o `\cdot`) en fórmulas.** Búscalo con Grep. Si un factor va entre paréntesis o es una fracción, quita el punto y yuxtapón (`v_N(1+r)^{-N}`, no `v_N \cdot (1+r)^{-N}`; `c\dfrac{1-(1+r)^{-N}}{r}`, no `c \cdot \dfrac{1-(1+r)^{-N}}{r}`). Dos símbolos de una sola letra también se yuxtaponen (`rta`, no `r \cdot t \cdot a`). Solo si yuxtaponer sería ambiguo (dos números sueltos sin paréntesis, o un símbolo de varias letras junto a otro símbolo, ej. `TNA` y `t`) usa `×` en vez del punto. No toques el "·" decorativo del título del archivo (`# Unidad N · Título`): esa regla es solo para multiplicación en fórmulas. Ver la regla en "Notación matemática" de `notas-unidad`.
7. **Español primero, inglés entre paréntesis.** Busca menciones de un término en inglés (o su sigla) que no vayan precedidas del término en español. Si el término en español no aparece en absoluto (solo el inglés), agrégalo antes: "fijación de precio (pricing)", no solo "pricing"; "valor presente (present value, PV)", no "esto es lo que en inglés se llama *present value*". Ver la regla en "Estilo de prosa" de `notas-unidad`.

8. **`π` para inflación, y superíndice `^e` para valores predichos.** Busca `\pi` o `π` usado como tasa de inflación y reemplázalo por `r_{inf}` (la letra griega queda reservada para la constante matemática). Busca un superíndice `^e` marcando un valor esperado/predicho (ej. `r_{inf}^e`, `r^e`) y reemplázalo por acento circunflejo (`\hat{r}_{inf}`, `\hat{r}`); el símbolo sin acento queda para el valor realizado/observado. Ver la regla de "familia de tasas" y "valor esperado/predicho" en "Notación matemática" de `notas-unidad`.

9. **Formato Markdown y tablas.** Si el proyecto tiene `markdownlint`/`prettier` instalados, córrelos y aplica sus fixes. Si no (como en este repo, sin `node_modules`), revisa a mano: encabezados con salto de nivel correcto y una línea en blanco antes/después, listas rodeadas de línea en blanco, sin tabs ni espacios al final de línea, el archivo termina en un solo salto de línea, y cada tabla tiene sus columnas alineadas (ancho de la celda más larga, `|` alineados entre filas, al menos un espacio de margen a cada lado del texto). No aplica `MD013` (longitud de línea): las tablas y párrafos largos son parte del estilo de estas notas.

## Qué reportar al usuario

Al terminar, resume en pocas líneas: cuántos em dashes se cambiaron y a qué, si se movieron citas a la sección de Fuentes, qué oraciones se recortaron por redundantes, qué símbolos cambiaron de mayúscula a minúscula (o viceversa) y por qué, cuántos puntos de multiplicación se quitaron, y si se reemplazó `π` por `r_{inf}` o un superíndice `^e` por `\hat{}` (con un ejemplo breve de antes/después si algún cambio no es obvio). No hace falta un reporte exhaustivo línea por línea.
