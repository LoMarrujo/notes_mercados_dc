...
- **Sin punto de multiplicación ("·" o `\cdot`); yuxtapón los factores cuando eso no sea ambiguo.** Si un factor va entre paréntesis o es una fracción, el punto sobra: escribe `v_N(1+r)^{-N}`, no `v_N \cdot (1+r)^{-N}`; `c\dfrac{1-(1+r)^{-N}}{r}`, no `c \cdot \dfrac{1-(1+r)^{-N}}{r}`. Dos símbolos de una sola letra también se yuxtaponen sin punto (`rta`, no `r \cdot t \cdot a`; `rs_{t-1}`, no `r \cdot s_{t-1}`). La única excepción es cuando yuxtaponer sí sería ambiguo: dos números sueltos sin paréntesis (`500,000 × 0.117459`; yuxtaponerlos los fundiría en un solo número sin sentido), o un símbolo de varias letras junto a otro símbolo (`TNA × t`, no `TNAt` ni `TNA \cdot t`, porque no se distinguiría dónde termina `TNA` y empieza `t`); ahí usa `×`, nunca el punto.
- **Familia de tasas con subíndice descriptivo, en vez de una letra griega por concepto.** Cuando una tasa se descompone o se especializa (nominal, periódica, continua, real, de inflación), usa `r` con un subíndice en minúsculas que describa el concepto, siguiendo el patrón ya establecido en `unidad1/4_ciencia_inversion.md`: `r_{nom}` (tasa nominal a la escala de $t$), `r_{per}` (tasa que se aplica en cada sub-periodo de capitalización), `r_{cont}` (tasa de capitalización continua), `r_{real}` (tasa efectiva real, la TEA ya descontada por inflación), `r_{inf}` (tasa de inflación del periodo). **Nunca uses `π` para inflación**, aunque sea la convención de otros textos: en estas notas `π` ya está reservada para la constante matemática (3.14159...), y reusarla para una tasa financiera es ambiguo.
- **Un valor esperado/predicho lleva acento circunflejo (`\hat{}`), nunca un superíndice `^e`.** Cuando una cantidad se calcula con una proyección o expectativa en vez del valor ya realizado/observado (una tasa de inflación esperada, un rendimiento predicho), márcala con `\hat{}` sobre el símbolo base: `$\hat{r}_{inf}$` es la inflación esperada, `$\hat{r}_{real}$` la tasa efectiva real predicha con ella. El mismo símbolo sin el acento (`$r_{inf}$`, `$r_{real}$`) es siempre el valor realizado u observado. El generador de diapositivas (`generar-diapositivas`) ya traduce `\hat{x}` y `\hat x` a un acento circunflejo unicode combinante tanto en fórmulas `$$...$$` como en texto corrido `$...$`.
...
- **Una fórmula se integra a la oración que la presenta, no a un aparato aparte.** Sin dos puntos antes si la oración sigue después de la fórmula, con punto final si ahí termina. La etiqueta en negritas "**Justificación matemática.**" es solo para una derivación que necesita su propio párrafo; si la explicación cabe en una frase, va en la misma prosa, sin la etiqueta. Cita o enlaza una fuente (o una sección de otro archivo) una sola vez por archivo; después refiérete a ella en prosa plana, sin repetir el enlace ni una atribución como "en palabras de X" cada vez que se vuelva a usar.
